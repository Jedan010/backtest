...
from DatabaseHelper import Mysql_helper, Mongo_helper
...
class HistoricMysqlDataHandler(DataHandler):
    ...
    def __init__(self, events, mysql_dir, symbol_list, DATABASE="MYSQL"):
        """
    ...
    def _open_convert_mysql(self):
        ...
        for s in self.symbol_list:
            if self.DATABASE == "MYSQL":
                self.symbol_data[s] = get_data_from_mysql(s)
                
            elif self.DATABASE == "MONGO":
                self.symbol_data[s] = get_data_from_mongo(s)
...
def get_data_from_mysql(symbol):
    ...
    data = mysql_helper.read(symbol)

    return data

def get_data_from_mongo(symbol):
    """
    从 MongoDB 中读取数据，输出 DataFrame 格式的时间
    """

    mongo_helper = Mongo_helper()
    mongo_helper.table = symbol
    data = mongo_helper.read_df()
